...
class CLinear(nn.Module):
    ...
    def __init__(self, weight, bias, device):
        ...
        self.weight = compress(weight.data.to(
            device), default_compression_config)
        self.bias = bias
...
def compress(tensor, config):
    ...
    # Pad
    pad_len = (
        group_size - original_shape[group_dim] % group_size) % group_size
    if pad_len != 0:
        pad_shape = original_shape[:group_dim] + \
            (pad_len,) + original_shape[group_dim+1:]
        tensor = torch.cat([
            tensor,
